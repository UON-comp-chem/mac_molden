...
c     12   mopac xyz input file 
      ...
      if (lenfn.eq.0.or.filenm.eq.' ') then
         ...
         if (iapp.le.1) then
             lc = linlen(curdir)
             if (icmdf.eq.1) then
                 tfile = curdir(1:lc)//'/'//filenm(1:lenfn)
                 ln = linlen(tfile)
             else
                 if (lenfn.le.1) then
                    ln = linlen(filenm)
                 ...
                 if (filenm(1:1).eq.'/') then
                    ...
                    tfile = curdir(1:lc)//'/'//filenm(1:ln)
                    ln = ln + lc + 1
                 endif
             endif
             if (iopt.eq.4.and.ixyz.eq.2) then
                open(unit=iun,form='unformatted',
     &          file=tfile(1:ln),status='unknown',action='write',
     &          err=2000)
             else
